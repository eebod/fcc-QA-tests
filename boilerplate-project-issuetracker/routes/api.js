  ...
  app
      ...
      try {
        ...
        const filteredIssues = issues.filter(issue => {
          return Object.keys(query).every(key => {
            if (key === "open") {
              return query[key] === "true" ? issue[key] : query[key] === "false" ? !issue[key] : true;
            }
    ...
    .put(function (req, res) {
      try {
        ...
        const data = readDataFromFile();
        const index = data.findIndex(issue => issue.project === project && issue._id === _id);

        if (index === -1) {
          return res.json({ error: "could not update", _id: _id });
        }

        ["issue_title", "issue_text", "created_by", "assigned_to", "status_text", "open"].forEach(key => {
